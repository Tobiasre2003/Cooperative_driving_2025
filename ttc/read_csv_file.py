...
def get_paths(files:list[str]):
    file_data = get_data(files, ['x', 'y', 'theta', 'speed'])
    size = 0
    paths = {}
    for file_name in file_data.keys():
        x_list = file_data[file_name]['x']
        y_list = file_data[file_name]['y']
        t_list = file_data[file_name]['theta']
        s_list = file_data[file_name]['speed']
        time_list = file_data[file_name]['time']
        size = len(x_list)
        path = []
        
        for n in range(size):
            path.append([Point(x_list[n], y_list[n]), t_list[n], s_list[n], time_list[n]])
